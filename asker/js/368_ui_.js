...
$.fn.rolling2 = function(op) {
	...
	intv1 = [];
	function slide(obj,t){
		$.data(obj[0],'status','pause');
		if(op.direction=='top'||op.direction=='bottom') {
			op.size = obj.find('li:first').height();
		} else if(op.direction=='left'||op.direction=='right') {
			op.size = obj.find('li:first').width();
		};
		obj.find('li').css('top','auto').css('right','auto').css('bottom','auto').css('left','auto').css(op.direction,op.size);
		obj.find('li.no-'+$.data(obj[0],'no')).css(op.direction,'0');
		if(t=='prev') {
			eval('obj.find(\'li.no-\'+$.data(obj[0],\'no\')).show().animate({'+op.direction+':op.size},op.speed,\'easeInOutCubic\',function(){$(this).css({'+op.direction+':-op.size});})');
			var __no = ($.data(obj[0],'no')*1)-1;
			$.data(obj[0],'no',__no);
			if($.data(obj[0],'no')<1) { $.data(obj[0],'no',obj.find('li').length); };
			eval('obj.find(\'li.no-\'+$.data(obj[0],\'no\')).css({'+op.direction+':-op.size}).stop().animate({'+op.direction+':0},op.speed,\'easeInOutCubic\',function(){$.data(obj[0],\'status\',\'play\');})');
		} else {
			eval('obj.find(\'li.no-\'+$.data(obj[0],\'no\')).show().animate({'+op.direction+':-op.size},op.speed,\'easeInOutCubic\',function(){$(this).css({'+op.direction+':op.size});})');
			var __no = ($.data(obj[0],'no')*1)+1;
			$.data(obj[0],'no',__no);
			if($.data(obj[0],'no')>obj.find('li').length) { $.data(obj[0],'no',1); };
			eval('obj.find(\'li.no-\'+$.data(obj[0],\'no\')).css({'+op.direction+':op.size}).stop().animate({'+op.direction+':0},op.speed,\'easeInOutCubic\',function(){$.data(obj[0],\'status\',\'play\');})');
		};
	}
	function fade(obj,t){
		...
		obj.find('li').css('top',0).css('right','auto').css('bottom','auto').css('left',0).end().find('li:visible').fadeOut(op.speed*2,'easeInOutCubic');
		obj.find('li.no-'+$.data(obj[0],'no')).fadeOut().delay(op.speed/2).fadeIn(op.speed*4,'easeInOutCubic',function(){$.data(obj[0],'status','play');});
	}
	return this.each(function(idx){
		var __this = $(this);
		$.data(__this[0],'status','play');
		$.data(__this[0],'no',op.start);
		__this.find('li').each(function(idx){$(this).addClass('no-'+(idx+1));});
		eval(op.ani+'(__this)');
		if(__this.find(op.btn_prev).length>0) {
			__this.find(op.btn_prev).bind('click',function(e){ e.preventDefault();
				if($.data(__this[0],'status')=='play') eval(op.ani+'(__this,\'prev\')');
			});
			__this.find(op.btn_next).bind('click',function(e){ e.preventDefault();
				if($.data(__this[0],'status')=='play') eval(op.ani+'(__this,\'next\')');
			});
		};
		if(op.interval) {
			intv1[idx] = setInterval(function(){
				eval(op.ani+'(__this)');
			}, op.interval);
			__this.bind('mouseenter',function(){clearInterval(intv1[idx]);}).bind('mouseleave',function(){intv1[idx] = setInterval(function(){
				eval(op.ani+'(__this)');
			}, op.interval);});
		};
	});
...
$.promotion.settings = {
	...
	debug: false,
	sub: false,
	json: "../js/promotion.json"
};
$.promotion.init = function () {
	if (!$.promotion.settings.debug) {
		var e = $(window.location.hostname).selector;
		if (e.search(/bugs\.co\.kr/g) > 0) {
			$.promotion.settings.channel = "B";
		} else if (e.search(/cyworld\.com/g) > 0) {
			$.promotion.settings.channel = "CM";
		} else if (e.search(/naver\.com/g) > 0) {
			$.promotion.settings.channel = "PN";
		} else if (e.search(/daum\.net/g) > 0) {
			$.promotion.settings.channel = "D";
		} else {
			$.promotion.settings.channel = "P";
		};
		try {
			if (pmangpub.title.search(/dev|dq/g) > 0) {
				$.promotion.settings.srl = $.promotion.settings.srl_dev;
			};
		} catch (t) {};
	};
};

$.promotion.make = function (e) {
	return $.each(e.promotion.item, function (i, v) {
		$.promotion.data = $.promotion.data || {};
		$.promotion.data[i] = $.promotion.data[i] || {};
		if ($.promotion.settings.sub) {
			$.promotion.data[i].m_img = v.s_img;
		} else {
			$.promotion.data[i].m_img = v.m_img;
		};
		$.promotion.data[i].extra1 = v.extra1;
		$.promotion.data[i].url = v.url;
		$.promotion.data[i].target = v.target;
		$.promotion.data[i].title = v.title;
		$.promotion.data[i].contents = v.contents;
	});
};
	
$.promotion.run = function (func) {
	if ($.promotion.settings.debug) {
		$.getJSON($.promotion.settings.json, function (e) {
			$.promotion.make(e);
			if ($.isFunction(func)) {
				func($.promotion.data)
			}
		...
		$.promotion.init();
		gamepub.getPromotionBanner($.promotion.settings.ssn, $.promotion.settings.srl, $.promotion.settings.channel, function (json) {
			json = eval("(" + json + ")");
			$.promotion.make(json);
			if ($.isFunction(func)) {
				func($.promotion.data)
	...
	return this.each(function(er){
		$.ndl[er] = $.ndl[er] || {
			...
			init			: function(){
								...
								.append('<p class="'+settings.nav+'" />');
							},
			control			: function(){
								if(settings.control==true) {
									$($.ndl[er].selector)
										.prepend('<a class="'+settings.prev+'"/><a class="'+settings.next+'"/>');
								};
							},
			ring			: function(){
								...
								$($.ndl[er].selector).children('p:first').find('a').each(function(idx){
									$(this).addClass(settings.item+(idx+1));
								});
							},
			print			: function(n,i1,u,t){
								this.n = n, this.i1 = i1, this.u = 'href="'+u+'"', this.t = 'target="'+t+'"';
								if(u=='#'||!u) { this.u = '', this.t = ''; };
								this.append = function(){
									$('<li><a '+this.u+' '+this.t+'><img src="'+this.i1+'" alt="" /></a></li>')
										.appendTo($($.ndl[er].selector).find('div.'+settings.list+' ul'));
									$('<a><span>'+(this.n+1)+'</span></a>')
										.appendTo($($.ndl[er].selector).find('p.'+settings.nav));
								};
							},
			parse			: function(xml){
								this.init();
			...
			on				: function(){
								$($.ndl[er].selector).find('p.'+settings.nav)
			...
			play			: function(){
								...
								if($.ndl[er].total()>0) {
									...
									$($.ndl[er].selector).find('p.'+settings.nav+' a').bind(settings.focus,function(){
		...
		if(settings.xml) {
			...
			$.ndl[er].control();
			$.ndl[er].play();
			$('.sales .nav a').bind('mouseenter',function(){
				var span = $(this).find('span:first');
				span.stop().animate({'margin-top':-3,height:14},50,'easeOutBack').animate({'margin-top':0,height:8},300,'easeOutQuint');
			});
		};
	});
};
...
var hostname = $(window.location.hostname).selector;
if (hostname.search(/bugs\.co\.kr/g) > 0) {
	$.ak.ch = "B";
} else if (hostname.search(/cyworld\.com/g) > 0) {
	$.ak.ch = "CM";
} else if (hostname.search(/naver\.com/g) > 0) {
	$.ak.ch = "PN";
} else if (hostname.search(/daum\.net/g) > 0) {
	$.ak.ch = "D";
} else {
	$.ak.ch = "P";
};
...
$.fn.rolling = function(op) {
	...
	return this.each(function(idx){
		...
		if(op.interval) {
			intv2[idx] = setInterval(function(){
				eval(op.ani+'(__this)');
			}, op.interval);
			__this.bind('mouseenter',function(){clearInterval(intv2[idx]);}).bind('mouseleave',function(){intv2[idx] = setInterval(function(){
				eval(op.ani+'(__this)');
			}, op.interval);});
		};
	});
														...
														mergeClassName: function(name, suffix) {
																...
																getSelectCSS: function(sel) {
																		var $sel = $(sel),
																			...
																			result.width = Number(result.width.slice(0,-2))+39; // 2014.10 mas
																			return result;
																		}
															};
...
$.ak.nav =  function(){
	$('.nav_in .dep1').bind("mouseenter", function(){		
		$('#ak_nav').addClass('on').animate({'height':'259px'},300);
		$('#ak_nav .nav_in').css({'height':'195px'});
	});
	$('#ak_nav').bind("mouseleave", function(){
		$('#ak_nav').animate({'height':'80px'},200,function(){
			$('#ak_nav .nav_in').css({'height':'30px'});
			$(this).removeClass('on');	
		});
	});
	return false;
...
$.ak.focus =  function(){
	$('.footer_in .pcbang .search input').bind("focusin",function(){
			$(this).attr({"placeholder":""});				
		}).bind("focusout",function(){			 
			$(this).attr({"placeholder":"ASKER PC방을 찾아보세요"});
		});	
		$('.footer_in .pcbang .search input').bind("focusin",function(){
			$(this).attr({"placeholder":""});		
			$(this).addClass('none');
		});	
		$('.loginbox .login .loginform input').bind("focusin",function(){
			$(this).attr({"placeholder":""});				
		}).bind("focusout",function(){			 
			$('input.login_id').attr({"placeholder":"아이디 또는 이메일"});
		}).bind("focusout",function(){			 
			$('input.login_pass').attr({"placeholder":"비밀번호"});
		});	
		$('.layout_sub input.text').bind("focusin",function(){
			$(this).attr({"placeholder":""});				
		}).bind("focusout",function(){			 
...
$.ak.shop = function(){	
	$('.shop .li_shop .thum, .shop_v2 .li_shop .thum').on('mouseover', function(){
		$(this).parents('dl').find('.option, .btn_more, .descript').css({"display":"block"});
	});	
	$('.shop .li_shop .option, .shop_v2 .li_shop .option').on('mouseleave', function(){
		$(this).parents('dl').find('.option, .btn_more, .descript').css({"display":"none"});
	});	
	$('.flexslider').flexslider({
		animation: "slide",
		animationLoop: false,
		slideshow: false
		//start: function($slider){ $.bl.slider = $slider; }			
	});
	$('.flex-prev, .flex-next').on('click',function(){
		var prevbtn = $(this).closest('.flex-direction-nav').find('.flex-prev');
		var nextbtn = $(this).closest('.flex-direction-nav').find('.flex-next');
		if($(prevbtn).hasClass('flex-disabled') && $(nextbtn).hasClass('flex-disabled')){
			alert('더 이상 상품이 없습니다.');			
		}
	});	
};
$.ak.btntoggle = function(){
	$('.bbs_comments .btn_toggle').bind('click',function(){
		$(this).parents('.bbs_comments').find('.list').toggle();
		return false;
	});
};
$.ak.slot_item = function(){
	$('.slot_item .item.end').prepend('<i></i>');
	$('.slot_item .item').bind("mouseover",function(){
		//$('.slot_item .item').css({"z-index":-1});
		$(this).parent().css({"z-index":1});
		$(this).parent().children(".preview").css({"display":"block"});
	});
	$('.slot_item .item').bind("mouseleave",function(){
		//$('.slot_item .item').css({"z-index":1});
		$(this).parent().css({"z-index":0});
		$(this).parent().children(".preview").css({"display":"none"});
	})
};
$.ak.slide_bn = function (){
	jQuery(document).ready(function($) {
		/* bjqs */
		if(($('.down.mobile').length > 0)) {
			$('.slide_mobile').bjqs({animtype : 'slide', automatic : true, height: 973,		width         : 356,		animspeed : 4000,		randomstart   : false	  });
		};		
	});
}
function set_lmsg_location() {
	$('.pop_lmsg').each(function(){
		$(this).css({
			/*
			top : parseInt($(window).scrollTop()+($(window).height()/2-$(this).height()/2)) + 'px',
			left : parseInt($(window).scrollLeft()+($(window).width()/2-$(this).width()/2)) + 'px'*/
			top : parseInt($(window).scrollTop()+($(window).height()/2-$(this).height()/2)) + 'px'
			//left : parseInt(($(this).width()-110)) + 'px'
		});
	});
	check_lmsg_curtain();
}
function check_lmsg_curtain() {
	if ($('.pop_lmsg:visible').length < 1) {
		$('#curtain').hide();
	} else if ($('.pop_lmsg:visible').length > 0) {
		$('#curtain').show();
	}
	$('#curtain').css({
...
function lmsg_close(target) {	
	var $target;	
	if(event) {
		$(document).one('click',function(event){
			check_lmsg_curtain();
			$target = $('.pop_lmsg').has($(event.target));
			$target.hide();
			$('#curtain').hide();
		});
	}
}
function lmsg_open(target) {
	check_lmsg_curtain();
	$(target).show();
}
//메인 모달레이어
...
(function($) {
	$.ak.banner = $.ak.banner || function() {
		var update = $('.update_banner'),
			close = $('#close');
		close.appendTo('.update_banner');
		TweenLite.set(update, { autoAlpha: 0, display: 'block', ease: Expo.easeOut}, 0);
		TweenLite.to(update, 1, { autoAlpha: 1, ease: Expo.easeOut}, 0.1);;
	}
	$(function() {
		// all
			...
			$.ak.nav();	
			$.ak.focus();	
			$('#zoom_media, .zoom_media').fancybox({});
			$('.layer_modal').fancybox({'hideOnContentClick': false});
			$('.layer_modal .btn_close, a#fancybox-close, a.btn_close').click(function(){
				$('a#fancybox-close').click();
			});
			$('.logout.crt_yes .name').click(function(){
					$(this).parent().find('.info_crt_yes').toggle();
					return false;
			});	
		// main
			$('.pcbang .coupon').rolling2({ start:0 });		
			$('.update').bind('click',function(e){
				e.preventDefault();
				$.ak.banner();
			});
			$('<a href="#" id="close">CLOSE</a>').appendTo('.layout_main  .header_in');
			$('#close').unbind().bind('click',function(e){
				e.preventDefault();
				var update = $('.update_banner');
				TweenLite.to(update, 1, { autoAlpha: 0, ease: Expo.easeOut, onComplete:function(){
					update.hide();
				}});
			});
			$('.cover2').appendTo('.mainbanner .banner');		
		// sub
			$('.box_select .ti').click(function(){
				$(this).parent().find('.list').toggle();
				return false;
			});	
			$('.sales').slideshow();
			$( ".sub_tl_box" ).click(function() {
			  $('.pb_v2').toggleClass('on');
			  return false;
			});
			//$('.select, .SelectHelpdesk').selectbox();
			//$('.nui-selectbox-select').hide();
			
		// main
		if($('body').hasClass('layout_main')){
			var ytBtn = $('.multimedia .movie a');
			   ytBtn.on({
				 click: function () {
			...
			$('.layout_main .monster_in').rolling({ start:0 });
		}
		// sub
		if($('body').hasClass('layout_sub')){
			$.ak.shop();
			//$.ak.slot_item();		
			//$.ak.btntoggle();
			$.ak.slide_bn();
		...
		if (typeof pmangpub !== 'undefined' && pmangpub.setOnloadView) {
			pmangpub.setOnloadView(function(){
				var helpdeskOption = {
					jQueryUI: 'http://file.pmang.kr/images/pmang/holeinone/cb/js/jquery-ui-1.8.18.custom.min.js', 
					datepickerBtn: 'http://file.pmang.kr/images/pmang/holeinone/cb/img/helpdesk/btn_datepicker.gif'
				};
				$.helpdeskReady(helpdeskOption);
				$('.box .crt .name, .box_select .ti').click(function(){
					$(this).parent().find('.info_crt_yes, .box_scroll, .list').toggle();
					return false;
				});	
				$('.guide_preset_table .guide_td a').click(function() {
		...
		set_lmsg_location();
		/* 프로모션 */
		if($('.layout_main,.layout_sub').length>0) {
			if (typeof pmangpub === 'undefined') {
				$.promotion.settings.debug = true;
			}
			$('body').hasClass('layout_sub') ? $.promotion.settings.sub = true : $.promotion.settings.sub = false;		
			$.promotion.init();
			$.promotion.run(function(data){
				var $banner_html='';
				var $paging_html='';
				var $list_html='';
				$.each(data,function(i,v){
					$banner_html += '<li id="rollBanner'+i+'"><a href="'+v.url+'" target="'+v.target+'"><img src="'+v.m_img+'" alt="'+v.title+'"></a></li>';
					$paging_html += '<li><a href="#rollBanner'+i+'">'+v.title+'</a></li>';
					$list_html += '<li><i class="shadow"></i><div><a href="'+v.url+'" target="'+v.target+'"><img src="'+v.extra1+'" alt="'+v.title+'"><br>'+v.title+'</a></div></li>';
				});
				var bannerClass = $('div.banner ul.slides');
				var pagingClass = $('div.banner ol');
				bannerClass.html($banner_html).find('li:eq(0)').addClass('on');
				pagingClass.html($paging_html).find('li:eq(0)').addClass('on');
				bannerClass.find('li').animate({opacity:0},0).css('display','block');
				bannerClass.find('.on').animate({opacity:1},0);

				var navLength = pagingClass.innerWidth();
				var liSize = pagingClass.find('li').innerWidth()*3;
				var nowNumber = 0;
				var nowPosition = 0;
				var maxNumber = pagingClass.find('li').size();
				var maxPosition = 3; // 노출네비개수
				
				$('div.banner .controls').find('span.count').html('<b>'+(nowNumber+1)+'</b>/'+maxNumber);

				pagingClass.find('a').click(function(){ 				
					bannerClass.find('.on').removeClass('on');
					pagingClass.find('.on').removeClass('on');
					$(this).parent().addClass('on');
					var href = $(this).attr('href');
					href = href.substring(href.indexOf('#'));
					$(href).addClass('on');
					bannerClass.find('li').animate({opacity:0},{ queue:false , easing:'easeOutQuart', duration:500 });
					bannerClass.find('.on').animate({opacity:1},{ queue:false , easing:'easeOutQuart', duration:500 });

					// 목록이동
					var indexLink = $(this).parent();
					nowNumber = $('.mainbanner .banner ol li').index(indexLink);
					 nowPosition = Math.floor(nowNumber/3);
					pagingClass.css('marginLeft',-nowPosition * liSize);
					$(this).parents('div.controls').find('span.count').html('<b>'+(nowNumber+1)+'</b>/'+maxNumber);
					return false;
				});

				$('.mainbanner .banner div.controls>a').click(function(){				
					if ( $(this).hasClass('prev')) { 
						nowNumber = nowNumber - 1;
						if ( nowNumber < 0 ) { nowNumber = pagingClass.find('li').length-1 };
					}
					else if ( $(this).hasClass('next')) { 
						nowNumber = nowNumber + 1; 
						if ( nowNumber > pagingClass.find('li').length-1 ) { nowNumber = 0 };
					}
					pagingClass.find('a:eq('+nowNumber+')').click();
					return false;
				});

				function clock(){
					if ( pagingClass.children('.on').next('li').length > 0 ) { pagingClass.children('.on').next('li').children('a').click(); }
					else { pagingClass.children('li:first-child').children('a').click(); }
				}
				$('.mainbanner div.banner, .section_sub div.banner').mouseenter(function(){
					if(typeof(int) !== 'undefined') clearInterval(int);
				});
				$('.mainbanner div.banner, .section_sub div.banner').mouseleave(function(){
					int=setInterval(function(){clock()}, 4000);
				});
				/*$('div.banner a.all').click(function(){
					$(this).next().fadeIn(200, function(){
							$(this).prepend('<ul>'+$list_html+'</ul>');
						}).find('.btn_close').bind('click', function(){
							$(this).parent().find('ul').empty().end().hide();
							return false;
						});
					return false;
				});*/
		...
		(function(){
			var spStart = 0,
				spContainer = $('.shop ol'),
				spItem = spContainer.find('li'),
				__w = 425;
			if(spItem.length==1) {
				var __w = 525;
			} else if(spItem.length==2) {
				var __w = 475;
			}
			spItem.on('mouseenter',function(){
				var index = spItem.index($(this)),
					_this = $(this);
				TweenLite.to(spItem.eq(index), 0.5, { width: __w , ease: Power4.easeOut},0);
				TweenLite.to(spItem.not(_this), 0.5, { width: 50 , ease: Power4.easeOut},0);
				TweenLite.to(spItem.eq(index).find('img'), 0.5, { left: -50 , ease: Power4.easeOut},0);
				TweenLite.to(spContainer.find('img').not(_this.find('img')), 0.5, { left: 0 , ease: Power4.easeOut},0);
			});
			spItem.eq(0).trigger('mouseenter');
		})();
		(function(){
			$('.aside .utility li').on('mouseenter',function(){
				var _this = $(this);
				TweenLite.set(_this, { marginLeft: -180},0);
			}).on('mouseleave',function(){
				var _this = $(this);
				TweenLite.to(_this, 0.5, { marginLeft: 0 , ease: Expo.easeOut},0);
			});
		})();
		(function(){
			var update = $('a.update');
			update.text('');
			$('<i/><b/><u/>').appendTo(update);
			var red = update.find('i'),
				blue = update.find('b'),
				arr = update.find('u');
			var updateAni = new TimelineMax({repeat: -1});

			updateAni.to(arr,	 0.4, { marginTop: -5, ease: Expo.easeOut},0);
			updateAni.to(blue,	 0.4, { marginTop: -5, autoAlpha: 1, ease: Expo.easeOut},0);
			updateAni.to(red,	 0.4, { marginTop: -5, autoAlpha: 0, ease: Expo.easeOut},0);

			updateAni.to(arr,	 0.6, { marginTop: 3, ease: Expo.easeIn},0.4);
			updateAni.to(blue,	 0.6, { marginTop: 3, ease: Expo.easeIn},0.4);
			updateAni.to(red,	 0.6, { marginTop: 3, ease: Expo.easeIn},0.4);

			updateAni.to(arr,	 0.4, { marginTop: -5, ease: Expo.easeOut},1);
			updateAni.to(blue,	 0.4, { marginTop: -5, autoAlpha: 0, ease: Expo.easeOut},1);
			updateAni.to(red,	 0.4, { marginTop: -5, autoAlpha: 1, ease: Expo.easeOut},1);

			updateAni.to(arr,	 0.6, { marginTop: 3, ease: Expo.easeIn},1.4);
			updateAni.to(blue,	 0.6, { marginTop: 3, ease: Expo.easeIn},1.4);
			updateAni.to(red,	 0.6, { marginTop: 3, ease: Expo.easeIn},1.4);

			/*updateAni.to(blue,	 0.5, { autoAlpha: 0, ease: Power3.easeOut},0.5);
			updateAni.to(red,	 0.5, { autoAlpha: 1, ease: Power3.easeOut},0.5);

			updateAni.to(blue,	 0.5, { autoAlpha: 1, ease: Power3.easeOut},1.5);
			updateAni.to(red,	 0.5, { autoAlpha: 0, ease: Power3.easeOut},1.5);
			*/		
		})();		
	});		
...
(function($) {
	$.helpdeskReady = function(options) {		
		...
		// add ava ob3 UI fix
		$('.hd_mtm_option').find('input[type="text"]').each(function(){
			var elBg = '<span class="tbg"></span>',
			elHolder = '<span class="input_text"></span>'
			$(this).wrap(elHolder).before(elBg);			
		});
		$(window).load(function(){
			$('.tbg').css('position','absolute');
		});
